...
int main(int argc, char *argv[]) {
    ...
    // [1]载入源图像并将其转换为灰度图
    g_srcImage = imread("jianzhu2.jpg", 1);
    cvtColor(g_srcImage, g_grayImage, COLOR_BGR2GRAY);
    
    // [2]创建窗口和滑动条, 并进行显示和回调函数的初始化
    namedWindow(WINDOW_NAME, WINDOW_AUTOSIZE);
    createTrackbar("最大角点数", WINDOW_NAME, &g_maxCornerNumber, g_maxTrackerNumber, on_GoodFeaturesToTrack);
